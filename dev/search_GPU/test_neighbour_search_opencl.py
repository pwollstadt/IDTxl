...
import pytest
import numpy as np
from idtxl.neighbour_search_opencl import clFindKnn, knn_search
from test_estimators_cmi import opencl_missing
...
def test_knn_one_dim():
    ...
    err = clFindKnn(indexes, distances, pointset, pointset, knn_k,
    ...
    (indexes2, distances2) = knn_search(np.expand_dims(pointset, axis=1),
                                        n_dims, knn_k, theiler_t, n_chunks,
                                        gpu_id)
...
def test_knn_two_dim():
    ...
    (indexes2, distances2) = knn_search(pointset2, n_dims, knn_k, theiler_t,
                                        n_chunks, gpu_id)
...
def test_one_dim_longer_sequence():
    ...
    # Call low-level function.
    err = clFindKnn(indexes, distances, pointset, pointset, knn_k, theiler_t,
                    n_chunks, n_dims, n_points, gpu_id)
...
def test_one_dim_two_dim_arg():
    ...
    err = clFindKnn(indexes2, distances2, pointset2, pointset2, knn_k,
                    theiler_t, n_chunks, n_dims, signal_length, gpu_id)
    ...
    with pytest.raises(AssertionError):
        clFindKnn(indexes3, distances3, pointset3, pointset3, knn_k,
                  theiler_t, n_chunks, n_dims, signal_length, gpu_id)
