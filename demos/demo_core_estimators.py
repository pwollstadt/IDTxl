...
cmi = est.estimate(source_cor, target, source_uncor)
print("Estimated CMI: {0:.5f}, expected CMI: {1:.5f}".format(cmi[0], expected_mi))
...
print(
    "Estimated synergy (Sydney): {0:.5f}, expected synergy: ~1".format(
        pid_sydney["syn_s1_s2"]
    )
)
pid_tartu = est_tartu.estimate(x, y, z)
print(
    "Estimated synergy (Tartu): {0:.5f}, expected synergy: ~1".format(
        pid_tartu["syn_s1_s2"]
    )
)
