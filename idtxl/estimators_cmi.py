...
def jidt_kraskov_fast(self, var1, var2, conditional, opts=None):
    """Calculate conditional mutual infor with JIDT's Kraskov implementation.
    ...
    jarLocation = 'infodynamics.jar'
    if not hasattr(self, 'jvm_started'):
        self.jvm_started = jp.isJVMStarted()
        if not self.jvm_started:
            jp.startJVM(jp.getDefaultJVMPath(), '-ea', ('-Djava.class.path=' +
                        jarLocation))

    if not hasattr(self, 'calc'):  # TODO think about this
        calc_class = (jp.JPackage('infodynamics.measures.continuous.kraskov').
                      ConditionalMutualInfoCalculatorMultiVariateKraskov1)
        self.calc = calc_class()
    self.calc.setProperty('NORMALISE', normalise)
    self.calc.setProperty('k', kraskov_k)
    self.calc.setProperty('DYN_CORR_EXCL', theiler_t)
    self.calc.setProperty('NOISE_LEVEL_TO_ADD', noise_level)
    self.calc.setProperty('NUM_THREADS', num_threads)
    self.calc.setDebug(debug)
