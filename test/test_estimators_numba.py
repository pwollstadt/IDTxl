...
def test_mi_uncorrelated_gaussians():
    ...
    # -------- for debugging ---------- start -------------------------------
    #source_uncorr = source_uncorr * 1000000
    ...
    plt.plot(dist_numbaCPU[:, 0])
    plt.plot(dist_numbaCPU64[:, 0])
    plt.plot(dist_numbaCuda)
    plt.plot(dist_ocl)
    plt.show()
...
#           'NumbaCPU MI result: {3:.4f} nats; expected to be close to {4:.4f} nats for correlated '
...
#                         'MI estimation for uncorrelated Gaussians using the '
#                         'OpenCL estimator failed (error larger 0.05).')
#     assert np.isclose(mi_numbaCPU, mi_jidt, atol=0.05), (
#                         'MI estimation for uncorrelated Gaussians using the '
#                         'NumbaCPU estimator failed (error larger 0.05).')
#     assert np.isclose(mi_numbaCuda, mi_jidt, atol=0.05), (
#                         'MI estimation for uncorrelated Gaussians using the '
