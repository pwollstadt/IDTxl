...
def test_multivariate_te_corr_gaussian(estimator=None):
    ...
    print(
        (
            "Max. entropy binning - estimated TE: {0:5.4f}, analytical result: "
            "{1:5.4f}, error: {2:2.2f} % "
        ).format(estimated_res_max_ent, expected_res, diff_max_ent / expected_res)
    )
    print(
        (
            "Equal binning - estimated TE: {0:5.4f}, analytical result: "
            "{1:5.4f}, error: {2:2.2f} % "
        ).format(estimated_res_equal, expected_res, diff_equal / expected_res)
    )
...
def test_multivariate_te_lorenz_2():
    ...
    settings[
        "max_lag_target"
    ] = 1  # was 0 before, but this is no longer allowed by the estimator
...
def test_multivariate_te_mute():
    ...
    results_me = network_analysis.analyse_network(settings, data, targets=[1, 2])
    settings["discretise_method"] = "equal"
    results_eq = network_analysis.analyse_network(settings, data, targets=[1, 2])

    for t in [1, 2]:
        print(
            "Target {0}: equal binning: {1}, max. ent. binning: {2}".format(
                t,
        ...
        if (
            results_eq.get_single_target(t, fdr=False).selected_vars_sources
            ...
            assert np.isclose(
                ...
                rtol=0.05,
            ), "Target {0}: unequl results for both binning " "methods.".format(t)
        else:
            continue
