...
def test_pid_user_input():
    ...
    with pytest.raises(AssertionError):
        pid.analyse_single_target(settings={}, data=Data(), target=0, sources=[1, 2])
    ...
    with pytest.raises(RuntimeError):  # Test incorrect number of sources
        pid.analyse_single_target(
            settings=settings, data=data, target=2, sources=[1, 2, 3]
        )
    settings["lags_pid"] = [0, 0, 0]
    with pytest.raises(RuntimeError):  # Test incorrect number of lags
        pid.analyse_single_target(
            settings=settings, data=data, target=2, sources=[1, 3]
        )
...
def test_network_analysis():
    ...
    est_tartu = pid.analyse_network(
        settings=settings, data=data, targets=[0, 2], sources=[[1, 2], [0, 1]]
    )
    assert (
        0.9 < est_tartu._single_target[2]["syn_s1_s2"] <= 1.1
    ...
    est_sydney = pid.analyse_network(
        settings=settings, data=data, targets=[0, 2], sources=[[1, 2], [0, 1]]
    )
    assert (
        0.9 < est_sydney._single_target[2]["syn_s1_s2"] <= 1.1
...
def test_analyse_single_target():
    ...
    pid = BivariatePID()
    settings = {"pid_estimator": "TartuPID", "tail": "two", "lags_pid": [0, 0]}
    est_tartu = pid.analyse_single_target(
        settings=settings, data=data, target=2, sources=[0, 1]
    )
    assert (
        0.9 < est_tartu._single_target[2]["syn_s1_s2"] <= 1.1
    ...
    est_sydney = pid.analyse_single_target(
        settings=settings, data=data, target=2, sources=[0, 1]
    )
    assert (
        0.9 < est_sydney._single_target[2]["syn_s1_s2"] <= 1.1
