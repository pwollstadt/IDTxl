...
def test_optimization_Rudelt_bbc():
    ...
    assert (res.R_tot_CI[0] == None), "R_tot_CO_lo needs to be None"
    assert (res.R_tot_CI[1] == None), "R_tot_CO_hi needs to be None"
...
def test_optimization_Rudelt_bbc_nonaveragedR():
    print("\nTest optimization_Rudelt using bbc estimator on Rudelt data with non_avereage_R")
    data = Data_spiketime()             # initialise empty data object
    data.load_Rudelt_data()             # load Rudelt spike time data

    # Only limited value set for speeding up the test. Results are not important,
    # because it will only be checked if CI is calculated and is not None
    settings = {'embedding_past_range_set': [0.005, 0.62946, 1.25594],
                'number_of_bootstraps_R_max': 10,
                'number_of_bootstraps_R_tot': 10,
    ...
    res = results_bbc.get_single_process(0)
    assert (res.R_tot_CI[0] != None), "R_tot_CO_lo needs to be a floating point values"
    assert (res.R_tot_CI[1] != None), "R_tot_CO_hi needs to be a floating point values"
...
def test_optimization_Rudelt_bbc_multiple_processes():
    ...
    #assert (res2.tau_R != res1.tau_R), ("tau_R of the different processes are Identical. Needs to be different")

    print("\tpassed")


def test_optimization_Rudelt_bbc_output_image():
    print("\nTest optimization_Rudelt using bbc estimator on Rudelt data with output image")
    data = Data_spiketime()             # initialise empty data object
    data.load_Rudelt_data()             # load Rudelt spike time data
    ...
    settings = {
        ...
        'number_of_bootstraps_R_max': 10,
        'number_of_bootstraps_R_tot': 10,
        'auto_MI_bin_size_set': [0.01, 0.25],
        'estimation_method': 'bbc',
        'visualization': True,
        'output_path': os.path.abspath(os.path.dirname(sys.argv[0])),
        'output_prefix': 'systemtest_optimizationRudelt_image1'}
...
def test_optimization_Rudelt_bbc_no_autoMI_output_image():
...
if __name__ == '__main__':
    ...
    test_optimization_Rudelt_bbc_output_image()
    test_optimization_Rudelt_bbc_no_autoMI_output_image()
    test_optimization_Rudelt_bbc_nonaveragedR()
