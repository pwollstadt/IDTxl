...
# Skip test module if mpi4py is missing
mpi4py = pytest.importorskip("mpi4py")
# Skip if test is run with only one MPI rank
ensure_mpi = pytest.mark.skipif(
    mpi4py.MPI.COMM_WORLD.Get_size() <= 1,
    reason="Test requires more than one MPI process",
)
...
def test_checkpoint_resume():
    print("Test resuming from automatically generated checkpoint.")
    ...
    print("Testing if variables were added correctly")
    for i in range(len(targets)):
    ...
    print("Testing if analysis runs from resumed checkpoint")
