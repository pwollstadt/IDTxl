...
def test_pid_user_input():
    ...
    target_1 = np.logical_xor(s1, s2).astype(int)
    target_1 = np.logical_xor(s3, s4).astype(int)
    target = np.logical_xor(target_1, target_2).astype(int)
    data = Data(np.vstack((s1, s2, s3, s4, target)), 'ps', normalise=False)
