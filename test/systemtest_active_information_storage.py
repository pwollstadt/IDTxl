...
def test_active_information_storage_opencl():
    ...
    print(
        f"AIS for MUTE data proc 1: {results.get_single_process(1, fdr=False)['ais']}"
    )
    print(
        f"AIS for MUTE data proc 2: {results.get_single_process(2, fdr=False)['ais']}"
    )
    print(
        f"AIS for MUTE data proc 3: {results.get_single_process(3, fdr=False)['ais']}"
    )
...
def test_active_information_storage_jidt():
    ...
    results = network_analysis.analyse_network(settings, data, processes)
    print(
        "AIS for MUTE data proc 1: {0}".format(
            results.get_single_process(1, fdr=False)["ais"]
        )
    )
    print(
        "AIS for MUTE data proc 2: {0}".format(
            results.get_single_process(2, fdr=False)["ais"]
        )
    )
    print(
        "AIS for MUTE data proc 3: {0}".format(
