...
def test_discretise_max_ent():
    # Test 1D discretisation, on an example to contrast with the even bins discretisation:
    ...
    # Now test where our boundary has several values on it (all values on it go into lower bin):
    discretised = utils.discretise_max_ent(np.array([1,0.8,0.8,0]), 2)
    assert check_all_bools_true(discretised == np.array([1,0,0,0]))
    # Test 2D discretisation
    discretised = utils.discretise_max_ent(np.array([[1,0],[0.9,0.8],[0.8,0.8],[0,1]]), 2)
    assert type(discretised) == np.ndarray
    assert len(discretised.shape) == 2
    assert discretised.shape[0] == 4
    assert discretised.shape[1] == 2
    assert (type(discretised[0,0]) == np.int32) or (type(discretised[0,0]) == np.int64)
    assert check_all_bools_true_2d(discretised == np.array([[1,0],[1,0],[0,0],[0,1]]))
