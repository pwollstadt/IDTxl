...
def test_network_comparison():
    ...
    for inference in ["bmi", "mmi", "bmi", "bte", "mte"]:
        ...
        res = pickle.load(
            open(
                "{0}discrete_results_{1}_{2}.p".format(path, inference, estimator), "rb"
            )
        )
        ...
        for estimator in ["JidtGaussianCMI", "JidtKraskovCMI"]:
            ...
            for stats_type in ["dependent", "independent"]:
                print(
                    (
                        "\n\n\n######### Running network comparison on {0} "
                        "results ({1} estimator) on continuous data, {2} test.".format(
                            inference, estimator, stats_type
                        )
                    )
                )
...
def _verify_test(c_within, c_between, res):
    ...
    for comp, comp_type in zip([c_between, c_within], ["between", "within"]):
        ...
        assert (
            adj_mat_union.edge_matrix[tp]
        ).all(), "Missing union link in {} network comparison.".format(comp_type)
        assert (
            adj_mat_pval.weight_matrix[tp] < 1
        ).all(), "Wrong p-value in {} network comparison.".format(comp_type)
        assert (
            adj_mat_diff.edge_matrix[tp] > 0
        ).all(), "Missed difference in {} network comparison.".format(comp_type)
