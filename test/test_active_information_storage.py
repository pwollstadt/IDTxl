...
def test_return_local_values():
    ...
    for p in processes:
        ...
        if lais is np.nan:
            continue
        assert (
            type(lais) is np.ndarray
        ), "LAIS estimation did not return an array of values: {0}".format(lais)
        assert (
            lais.shape[0] == data.n_replications
        ), "Wrong dim (no. replications) in LAIS estimate: {0}".format(lais.shape)
        assert lais.shape[1] == data.n_realisations_samples(
            (0, max_lag)
...
def test_analyse_network():
    ...
    assert all(
        np.array(k) == np.arange(data.n_processes)
...
def test_single_source_storage_gaussian():
    ...
    print(
        "AIS for random normal data without memory (expected is NaN): "
        "{0}".format(results._single_process[1].ais)
    )
    assert (
        results._single_process[1].ais is np.nan
...
def test_compare_jidt_open_cl_estimator():
    ...
    print(
        "AIS for MUTE data proc 2 - opencl: {0} and jidt: {1}".format(
            ais_opencl_2, ais_jidt_2
        )
    )
    print(
        "AIS for MUTE data proc 3 - opencl: {0} and jidt: {1}".format(
    ...
    if not (ais_opencl_2 is np.nan or ais_jidt_2 is np.nan):
        assert (
            ais_opencl_2 - ais_jidt_2
        ) < 0.05, "AIS results differ between OpenCl and JIDT estimator."
    else:
        assert (
            ais_opencl_2 is ais_jidt_2
