...
def test_checkpoint_resume():
    ...
    for i, target in enumerate(targets):
        network_analysis._initialise(settings, data, sources, target)
        network_analysis.selected_vars_full = add_vars[i]
        network_analysis._update_checkpoint(f"{filename_ckp}.ckp")
    ...
    for i, target in enumerate(targets):
        network_analysis_res._initialise(settings, data, sources, target)
        assert (
            network_analysis_res.selected_vars_full == add_vars[i]
        ), f"Variables were not added correctly for target {target}."
...
def test_checkpoint_copy():
    ...
    assert os.path.isfile(
        f"{filename_ckp}.ckp"
    ), f"No checkpoint file {filename_ckp}.ckp was written."
    assert os.path.isfile(
        f"{filename_ckp}.ckp.old"
    ), f"No checkpoint file {filename_ckp}.ckp.old was written."
