...
def test_cmi_estimator_ocl():
    ...
    # Generate two sets of random normal data, where one set has a given
    # covariance and the second is uncorrelated.
    n = 4001  # needs to be odd as we loose one sample when shifting signals
    cov = 0.4
    source_1 = [rn.normalvariate(0, 1) for r in range(n)]  # correlated src
    source_2 = [rn.normalvariate(0, 1) for r in range(n)]  # uncorrelated src
    target = [sum(pair) for pair in zip(
...
def test_cmi_estimator_jidt_discrete():
    ...
    for t in range(n):
        ...
        target[t] = source_1[t]
    res_1a = est.estimate(var1=source_1, var2=target,
                          conditional=None, opts=opts)
    res_1b = est.estimate(var1=source_2, var2=target,
                          conditional=None, opts=opts)
    print('Example 1a: CMI result {0:.4f} bits; expected to be 1 bit for the '
          'copy (no conditional)'.format(res_1a))
    ...
    res_2a = est.estimate(var1=source_1, var2=target,
                          conditional=source_2, opts=opts)
    res_2b = est.estimate(var1=source_2, var2=target,
                          conditional=source_1, opts=opts)
    print('Example 2a: CMI result {0:.4f} bits; expected to be 1 bit for the '
          'copy (irrelevant conditional)'.format(res_2a))
    ...
    res_3 = est.estimate(var1=source_1, var2=target,
                         conditional=source_1, opts=opts)
    print('Example 3: CMI result {0:.4f} bits; expected to be 0 bit for the '
          'copy (conditional also copy)'.format(res_3))
    assert (res_3 == 0), ('CMI calculation for copy with copied conditional '
                          'failed.')

    # Test 4: target is an xor of the source and conditional
    res_4a = est.estimate(var1=source_2, var2=target,
                          conditional=source_3, opts=opts)
    res_4b = est.estimate(var1=source_3, var2=target,
                          conditional=source_2, opts=opts)
