...
def test_results_network_inference():
    ...
    target_1 = covariance * source + (1 - covariance) * np.random.normal(0, 1, size=n)
    target_2 = covariance * source + (1 - covariance) * np.random.normal(0, 1, size=n)
    ...
    data = Data(
        np.vstack((source_dis, target_1_dis, target_2_dis)),
        dim_order="ps",
    ...
    nw = BivariateTE()
    # TE - single target
    res_single_biv_te = nw.analyse_single_target(settings=settings, data=data, target=1)
    # TE whole network
    ...
    nw = BivariateMI()
    # TE - single target
    res_single_biv_mi = nw.analyse_single_target(settings=settings, data=data, target=1)
    # TE whole network
    res_network_biv_mi = nw.analyse_network(settings=settings, data=data)

    res_te = [
        res_single_multi_te,
        res_network_multi_te,
        res_single_biv_te,
        res_network_biv_te,
    ]
    res_mi = [
        res_single_multi_mi,
        res_network_multi_mi,
        res_single_biv_mi,
        res_network_biv_mi,
    ]
    ...
    n_nodes = 3
    n_realisations = (
        n - delay - max(settings["max_lag_sources"], settings["max_lag_target"])
    )
    for res in res_all:
        assert res.data_properties.n_nodes == n_nodes, "Incorrect no. nodes."
        assert res.data_properties.n_nodes == n_nodes, "Incorrect no. nodes."
        assert (
            res.data_properties.n_realisations == n_realisations
        ), "Incorrect no. realisations."
        assert (
            res.data_properties.n_realisations == n_realisations
        ), "Incorrect no. realisations."
        assert (
            res.data_properties.normalised == normalisation
        ...
        assert (
            adj_matrix.edge_matrix.shape[0] == n_nodes
        ), "Incorrect number of rows in adjacency matrix."
        assert (
            adj_matrix.edge_matrix.shape[1] == n_nodes
...
def test_pickle_results():
    ...
    nw = MultivariateTE()
    res_single = nw.analyse_single_target(settings=settings, data=data, target=1)
...
def test_add_single_result():
    ...
    data = _get_discrete_gauss_data()
    nw = MultivariateTE()
    res_network = nw.analyse_single_target(settings=settings, data=data, target=1)

    # Test adding target results that already exists
    with pytest.raises(RuntimeError):
        res_network._add_single_result(
            target=1, settings=res_network.settings, results={}
        )
    ...
    with pytest.raises(RuntimeError):
        res_network._add_single_result(
            target=0, settings=settings_test, results=res_network._single_target[1]
        )
    ...
    assert (
        "new_setting" in res_network.settings.keys()
    ), "Settings dict was not updated."
    assert (
        res_network.settings.new_setting == "Test"
...
def test_delay_reconstruction():
    ...
    target_1 = covariance * source + (1 - covariance) * np.random.normal(0, 1, size=n)
    target_2 = covariance * source + (1 - covariance) * np.random.normal(0, 1, size=n)
    target_3 = covariance * source + (1 - covariance) * np.random.normal(0, 1, size=n)
    ...
    assert (
        adj_mat.weight_matrix[0, 1] == delay_1
    ), "Estimate for delay 1 is not correct."
    assert (
        adj_mat.weight_matrix[0, 2] == delay_2
    ), "Estimate for delay 2 is not correct."
    assert (
        adj_mat.weight_matrix[0, 3] == delay_3
...
def test_results_network_comparison():
    ...
    for i, res in enumerate([res_within, res_between]):
        ...
        assert adj_matrix_union.edge_matrix[
            s, t
        ].all(), "{0}-test did not return correct union network links.".format(test[i])
        # Comparison
        assert adj_matrix_comp.edge_matrix[
            s, t
        ].all(), "{0}-test did not return correct comparison results.".format(test[i])
        no_diff = np.extract(
            np.invert(adj_matrix_union.edge_matrix), adj_matrix_comp.edge_matrix
        )
        assert (
            not no_diff.any()
        ), "{0}-test did not return 0 comparison for non-sign. links.".format(test[i])
        # Abs. difference
        assert (
            adj_matrix_diff.edge_matrix[s, t]
        ).all(), "{0}-test did not return correct absolute differences.".format(test[i])
        no_diff = np.extract(
            np.invert(adj_matrix_union.edge_matrix), adj_matrix_diff.edge_matrix
        )
        assert (
            not no_diff.any()
        ), "{0}-test did not return 0 difference for non-sign. links.".format(test[i])
        # p-value
        p_max = 1 / comp_settings["n_perm_comp"]
        assert (
            adj_matrix_pval.weight_matrix[s, t] == p_max
        ).all(), "{0}-test did not return correct p-value for sign. links.".format(
            test[i]
        )
        no_diff = np.extract(
            np.invert(adj_matrix_union.edge_matrix), adj_matrix_pval.edge_matrix
        )
        assert (
            not no_diff.any()
        ), "{0}-test did not return p-vals of 1 for non-sign. links.".format(test[i])
...
def test_adjacency_matrix():
    ...
    assert (
        adj_mat.weight_matrix[edge[0], edge[1]] == weight
    ), "Weighted edge was not added."
    assert adj_mat.edge_matrix[edge[0], edge[1]], "Edge was not added."
    ...
    for i in range(len(i_list)):
        assert (
            adj_mat.weight_matrix[i_list[i], j_list[i]] == weights[i]
        ), "Weighted edge was not added."
        assert adj_mat.edge_matrix[i_list[i], j_list[i]], "Edge was not added."
    adj_mat.add_edge_list(np.array(i_list), j_list, weights)

    # Test getting list of edges
    adj_mat.get_edge_list()

    # Test comparison of entry types-these should all pass
    a = AdjacencyMatrix(n_nodes, weight_type=bool)
    a.add_edge(0, 1, True)
    a = AdjacencyMatrix(n_nodes, weight_type=int)
    a.add_edge(0, 1, int(1))
    a.add_edge(0, 2, np.int32(1))
    a.add_edge(0, 3, np.int64(1))
