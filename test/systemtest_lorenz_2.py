"""Run test on two bi-directionally coupled Lorenz systems using CPU estimators
...
d = np.load(
    Path(os.path.dirname(__file__)).joinpath('data/lorenz_2_exampledata.npy')
)
...
with open(path.joinpath('test_lorenz_res_0'), 'wb') as output_file:
    pickle.dump(res_0, output_file)
with open(path.joinpath('test_lorenz_res_1'), 'wb') as output_file:
