...
def test_return_local_values():
    ...
    assert (
        lmi.shape[2] == data.n_replications
    ...
    mi_selected_sources = results_avg.get_single_target(target, fdr=False)[
        "selected_sources_mi"
    ]
    ...
    print(
        "Single link average MI: {0}, single source MI: {1}.".format(
    ...
    if mi_single_link is None:
        return
    assert np.isclose(
        mi_single_link, mi_selected_sources, atol=0.005
    ).all(), "Single link average MI {0} and single source MI {1} deviate.".format(
        mi_single_link, mi_selected_sources
    )
...
def test_zero_lag():
    ...
    expected_mi, source, source_uncorr, target = _get_gauss_data()
    data = Data(np.hstack((source, target)), dim_order="sp", normalise=False, seed=SEED)
    settings = {
        "cmi_estimator": "JidtKraskovCMI",
        "n_perm_max_stat": 21,
        "n_perm_min_stat": 21,
    ...
    print(
        "Estimated omnibus MI: {0:0.6f}, estimated MI using JIDT core "
    ...
    assert np.isclose(
        omnibus_mi, expected_mi, atol=0.05
...
def test_discrete_input():
    ...
    assert np.isclose(res._single_target[1].omnibus_mi, expected_mi, atol=0.05), (
        "Estimated MI for discrete variables is not correct. Expected: "
        "{0}, Actual results: {1}.".format(expected_mi, res["selected_sources_mi"][0])
    )
